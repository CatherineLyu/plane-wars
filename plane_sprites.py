...
class BackGround(GameSprite):
    ...
    def __init__(self, is_alt=False):
        ...
        # 判断是否为背景图像2，若是则改变初始坐标位置（即默认背景图像2的初始位置所在坐标应为游戏主窗口的正上方
        if is_alt:
...
class Enemy(GameSprite):
    ...
    def __init__(self):
        ...
        # 随机抽取出场位置
        self.rect.x = random.randrange(0, (SCREEN_RECT.width - self.rect.width))

        # 随机抽取出场速度
        self.speed = random.randint(1, 3)

        # 初始位置应该在游戏主窗口的上方
        # self.rect.bottom = 0 该代码等同 self.rect.y = -self.rect.height
